...
const render = nodeFn => {
  ...
  const spaces2 = Array(10 - node.x.toString().length)
    .fill(" ")
    .join("");
  const spaces3 = Array(20 - node.text.toString().length)
    .fill(" ")
    .join("");
  ...
  return {
    ...node,
    children: children ? children.map(node => render(node)) : []
  };
};
...
const Node = fn => {
  ...
  const _constructor = (props, initialState) => {
    const builder = () => fn(props, state || initialState, setState, store);
    const name = _constructor.displayName ? _constructor.displayName : "Node";
    Object.defineProperty(builder, "name", { value: name });
...
const Scene = Node((props, state = initialSceneState, setState) => {
  return {
    ...
    children: [
      N1({
        onStuffChanged: x => setState({ text: x })
      }),
